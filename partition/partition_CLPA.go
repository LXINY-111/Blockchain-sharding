...
// UpdatePrefixShard 是 AERO 专用的辅助函数
// 它将所有以 prefix 开头的账户移动到 newShard
func (cs *CLPAState) UpdatePrefixShard(prefix string, newShard int) {
	for v := range cs.NetGraph.VertexSet {
		if len(v.Addr) >= len(prefix) && v.Addr[:len(prefix)] == prefix {
			oldShard := cs.PartitionMap[v]
			if oldShard == newShard {
				continue
			}
			cs.PartitionMap[v] = newShard
			// 简单更新计数
			cs.VertexsNumInShard[oldShard]--
			cs.VertexsNumInShard[newShard]++
		}
	}
}
